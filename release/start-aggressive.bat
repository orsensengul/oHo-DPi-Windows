...
if not exist "%~dp0oHo-DPi.ps1" (
  ...
  echo [oHo-DPi] Sonra cikan klasordeki start-aggressive.bat dosyasina cift tikla.
  echo.
  pause
  exit /b 1
)
echo [oHo-DPi] Starting aggressive SpoofDPI profile. Npcap / wpcap.dll is required.
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0oHo-DPi.ps1" start-aggressive
echo.
